...
function generateFile(ctx, fileDesc) {
    ...
    if (options.outputEncodeMethods || options.outputJsonMethods || options.outputTypeRegistry) {
        ...
        `);
            if (options.outputTypeRegistry) {
                const messageTypeRegistry = (0, ts_poet_1.imp)('messageTypeRegistry@./typeRegistry');
                chunks.push((0, ts_poet_1.code) `
    ...
    if (options.outputServices === options_1.ServiceOption.DEFAULT && options.outputClientImpl && fileDesc.service.length > 0) {
        if (options.outputClientImpl === true) {
            chunks.push((0, generate_services_1.generateRpcType)(ctx));
        }
        else if (options.outputClientImpl === 'grpc-web') {
            chunks.push((0, generate_grpc_web_1.addGrpcWebMisc)(ctx, hasStreamingMethods));
        }
    }
    if (options.context) {
...
function makeLongUtils(options, bytes) {
    ...
    const util = (0, ts_poet_1.imp)('util@protobufjs/minimal');
    const configure = (0, ts_poet_1.imp)('configure@protobufjs/minimal');
    ...
    // not esModuleInterop.
    const Long = options.esModuleInterop ? (0, ts_poet_1.imp)('Long=long') : (0, ts_poet_1.imp)('Long*long');
...
function makeDeepPartial(options, longs) {
    ...
    const maybeLong = options.forceLong === options_1.LongOption.LONG ? (0, ts_poet_1.code) ` | ${longs.Long}` : '';
...
function makeTimestampMethods(options, longs) {
    const Timestamp = (0, ts_poet_1.imp)('Timestamp@./google/protobuf/timestamp');
...
function isOptionalProperty(field, options) {
    return (options.useOptionals && (0, types_1.isMessage)(field) && !(0, types_1.isRepeated)(field)) || field.proto3Optional;
}
...
function generateBaseInstance(ctx, fullName, messageDesc, fullTypeName) {
    const fields = messageDesc.field
        .filter((field) => !(0, types_1.isWithinOneOf)(field))
        .map((field) => [field, (0, types_1.defaultValue)(ctx, field)])
        .filter(([field, val]) => val !== 'undefined' && !(0, types_1.isBytes)(field))
        .map(([field, val]) => {
        const name = (0, case_1.maybeSnakeToCamel)(field.name, ctx.options);
        return (0, ts_poet_1.code) `${name}: ${val}`;
    });
    if (ctx.options.outputTypeRegistry) {
        fields.unshift((0, ts_poet_1.code) `$type: '${fullTypeName}'`);
    }
    return (0, ts_poet_1.code) `const base${fullName}: object = { ${(0, ts_poet_1.joinCode)(fields, { on: ',' })} };`;
}
...
function generateDecode(ctx, fullName, messageDesc) {
  ...
  `);
    ...
    messageDesc.field.filter(types_1.isRepeated).forEach((field) => {
        const name = (0, case_1.maybeSnakeToCamel)(field.name, options);
        const value = (0, types_1.isMapType)(ctx, messageDesc, field) ? '{}' : '[]';
        chunks.push((0, ts_poet_1.code) `message.${name} = ${value};`);
    });
    // initialize all buffers
    messageDesc.field
        .filter((field) => !(0, types_1.isRepeated)(field) && !(0, types_1.isWithinOneOf)(field) && (0, types_1.isBytes)(field))
        .forEach((field) => {
  ...
  `);
    ...
    messageDesc.field.forEach((field) => {
        ...
        if ((0, types_1.isPrimitive)(field)) {
            ...
            if ((0, types_1.isBytes)(field)) {
                ...
                if (options.stringEnums) {
                    const fromJson = (0, types_1.getEnumMethod)(typeMap, field.typeName, 'FromJSON');
            ...
            readSnippet = (0, ts_poet_1.code) `${type}.decode(reader, reader.uint32()).value`;
        }
        ...
        `);
                ...
                chunks.push((0, ts_poet_1.code) `message.${fieldName}.push(${readSnippet});`);
            }
            else {
                chunks.push((0, ts_poet_1.code) `
          if ((tag & 7) === 2) {
...
const Writer = (0, ts_poet_1.imp)('Writer@protobufjs/minimal');
  ...
  `);
    ...
    messageDesc.field.forEach((field) => {
        ...
        if ((0, types_1.isEnum)(field) && options.stringEnums) {
            ...
            const toNumber = (0, types_1.getEnumMethod)(typeMap, field.typeName, 'ToNumber');
            ...
            writeSnippet = (place) => (0, ts_poet_1.code) `${type}.encode({ ${maybeTypeField} value: ${place}! }, writer.uint32(${tag}).fork()).ldelim()`;
        }
        else if ((0, types_1.isMessage)(field)) {
            const tag = ((field.number << 3) | 2) >>> 0;
            const type = (0, types_1.basicTypeName)(ctx, field);
      ...
      `);
            ...
            chunks.push((0, ts_poet_1.code) `
        if (${(0, types_1.notDefaultCheck)(ctx, field, `message.${fieldName}`)}) {
          ${writeSnippet(`message.${fieldName}`)};
        }
      `);
        }
...
function generateFromJson(ctx, fullName, messageDesc) {
  ...
  `);
    ...
    messageDesc.field.forEach((field) => {
        ...
        const readSnippet = (from) => {
            if ((0, types_1.isEnum)(field)) {
                ...
                if ((0, types_1.isBytes)(field)) {
                    ...
                    return (0, ts_poet_1.code) `${cstr}.fromString(${from})`;
                }
                else {
                    const cstr = (0, case_1.capitalize)((0, types_1.basicTypeName)(ctx, field, { keepValueType: true }).toCodeString());
                ...
                if ((0, types_1.isRepeated)(field) && (0, types_1.isMapType)(ctx, messageDesc, field)) {
                    ...
                    if ((0, types_1.isPrimitive)(valueType)) {
                        // TODO Can we not copy/paste this from ^?
                        if ((0, types_1.isBytes)(valueType)) {
                            if (options.env === options_1.EnvOption.NODE) {
                        ...
                        else if ((0, types_1.isEnum)(valueType)) {
                            return (0, ts_poet_1.code) `${from} as number`;
                        }
                        else {
                            const cstr = (0, case_1.capitalize)((0, types_1.basicTypeName)(ctx, valueType).toCodeString());
                            return (0, ts_poet_1.code) `${cstr}(${from})`;
                        }
                    }
        ...
        // and then use the snippet to handle repeated fields if necessary
        chunks.push((0, ts_poet_1.code) `if (object.${fieldName} !== undefined && object.${fieldName} !== null) {`);
        if ((0, types_1.isRepeated)(field)) {
            if ((0, types_1.isMapType)(ctx, messageDesc, field)) {
      ...
      `);
            ...
            chunks.push((0, ts_poet_1.code) `message.${fieldName} = ${readSnippet(`object.${fieldName}`)};`);
        }
        ...
        if (!(0, types_1.isRepeated)(field) &&
            ...
            chunks.push((0, ts_poet_1.code) `} else {`);
            chunks.push((0, ts_poet_1.code) `message.${fieldName} = ${v};`);
        }
        chunks.push((0, ts_poet_1.code) `}`);
    });
    // and then wrap up the switch/while/return
...
function generateToJson(ctx, fullName, messageDesc) {
  ...
  `);
    ...
    messageDesc.field.forEach((field) => {
        ...
        const readSnippet = (from) => {
            if ((0, types_1.isEnum)(field)) {
                const toJson = (0, types_1.getEnumMethod)(typeMap, field.typeName, 'ToJSON');
                ...
                if ((0, types_1.isEnum)(valueType)) {
                    const toJson = (0, types_1.getEnumMethod)(typeMap, valueType.typeName, 'ToJSON');
                ...
                else if ((0, types_1.isScalar)(valueType)) {
                    return (0, ts_poet_1.code) `${from}`;
                }
                else {
                    const type = (0, types_1.basicTypeName)(ctx, valueType);
      ...
      `);
            ...
            chunks.push((0, ts_poet_1.code) `message.${oneofName}?.$case === '${fieldName}' && (obj.${fieldName} = ${v});`);
        }
        else {
            const v = readSnippet(`message.${fieldName}`);
            chunks.push((0, ts_poet_1.code) `message.${fieldName} !== undefined && (obj.${fieldName} = ${v});`);
        }
    });
    ...
    messageDesc.field.forEach((field) => {
      ...
      `);
            ...
            if (((0, types_1.isLong)(field) || (0, types_1.isLongValueType)(field)) && options.forceLong === options_1.LongOption.LONG) {
                ...
                const type = (0, types_1.basicTypeName)(ctx, field);
                chunks.push((0, ts_poet_1.code) `message.${fieldName} = ${v} as ${type};`);
            }
            else {
                chunks.push((0, ts_poet_1.code) `message.${fieldName} = ${readSnippet(`object.${fieldName}`)};`);
            }
        }
        if (!(0, types_1.isRepeated)(field) && options.oneof !== options_1.OneofOption.UNIONS) {
            chunks.push((0, ts_poet_1.code) `} else {`);
            const v = (0, types_1.isWithinOneOf)(field) ? 'undefined' : (0, types_1.defaultValue)(ctx, field);
            chunks.push((0, ts_poet_1.code) `message.${fieldName} = ${v}`);
        }
